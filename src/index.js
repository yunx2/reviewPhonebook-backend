...
app.post('/api/persons', (req, res) => {
  ...
  newPerson.save()
    .then(data => {res.status(201).json(data)}); // res.sendStatus method sends status code, then automatically ends response (like how res.send sends the response and then ends it); res.status method only sends the status code 
});
...
app.delete('/api/persons/:id', (req, res) => {  // not working
  const id = parseInt(req.params.id);
